...
#Devuelve editDistance y alineamiento optimo de dos sequencias
def editDistanceOptimalAlignment(seq1, seq2):
	m = len(seq1)
	n = len(seq2)
	if m<n:
		return editDistanceOptimalAlignment(seq2, seq1)
	...
	while i <= m:
		...
		while j <= n:
			...
			if seq1[i-1] != seq2[j-1]:
				val=1
			u=min(v[i-1][j]+1, v[i][j-1]+1, v[i-1][j-1]+val)
			if u== v[i-1][j-1]+val:
				v[i][j]=u
				tmp.append(((i-1),(j-1)))
			elif u==v[i-1][j]+1:
	...
	j = n
	while(i>0 or j>0):
		if i>0 and j>0 and (v[i][j] == v[i-1][j-1] or v[i][j]-1 == v[i-1][j-1]):
			alignmentA = alignmentA+seq1[i-1]
			alignmentB = alignmentB+seq2[j-1]
			i=i-1
			j=j-1
			print '1'
		elif i>0 and v[i][j]-1 == v[i-1][j]:
			alignmentA = alignmentA+seq1[i-1]
			alignmentB = alignmentB+'-'
			i=i-1
			print '2'
		elif j>0 and v[i][j]-1 == v[i][j-1]:
			alignmentA = alignmentA+'-'
			alignmentB =alignmentB+seq2[j-1]
			j=j-1
			print '3'
		print ''.join(reversed(alignmentA))
		print ''.join(reversed(alignmentB))
		print ':::::'
...
input_file = working_dir+'rosalind_edta.txt'
...
s = records[0].sequence
t = records[1].sequence
print s
print t
print pichurrea(s,t)
